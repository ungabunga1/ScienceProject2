...
public class Matrix {
   ...
   private void setEntries(double[][] entries) {
    ...
    public Matrix(double[] doubles, int rows) {

        if (doubles.length % rows != 0 || rows < 1) {
            throw new IllegalArgumentException("the double array cannot be evenly divided by the number of rows specified");
        }
        int columns = doubles.length/rows;
        double[][] newDoubles = new double[rows][columns];

        for (int i = 0; i < rows; i++) {
            int start = i * columns;
            int end = ((i + 1) * columns);
           newDoubles[i] = Arrays.copyOfRange(doubles, start,end);
        }
        setEntries(newDoubles);
    }
